...
async function loadProfile(user) {
  ...
  document.getElementById("nickname").textContent = `Name: ${myData?.nickname ?? ""}`
  document.getElementById("coins").textContent    = `💰 ZP: ${myData.coins ?? 0}`
...
window.apToggleAll = function(cb) {
  ...
  if (apIsAll) {
    ...
    document.getElementById("ap-tags").innerHTML =
      '<span style="font-size:13px;color:#4CAF50;">✅ 전체 유저에게 발송</span>'
  } else {
    apRenderTags()
  }
}
...
window.apAdd = async function() {
  ...
  if (!nickname) { showToast("닉네임을 입력해줘!"); return }

  const allSnap = await getDocs(collection(db, "users"))
  const found   = allSnap.docs.find(d => d.data().nickname === nickname)

  if (!found)                                      { showToast("그런 유저는 없어!"); return }
  if (found.id === myUid)                          { showToast("자기 자신은 안 돼!"); return }
  if (apRecipients.find(r => r.uid === found.id))  { showToast("이미 추가됐어!"); return }
...
function apRenderTags() {
  ...
  if (apRecipients.length === 0) {
    el.innerHTML = '<span style="color:#bbb;font-size:12px;">수신자를 추가해줘</span>'
    return
  }
...
window.apSend = async function() {
  ...
  try {
    ...
    showToast(`📮 ${targets.length}명에게 발송했어!`)
