...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await db.runTransaction(async (t) => {
      ...
      if (data.intro_done) return { skip: true }
      if (!data.first_slot) return { skip: true, noFirstSlot: true }
    ...
    const logs = [
      ...
      { text: "", type: "intro_wait", ts: ts++ }, // 클라이언트가 3초 대기용
      ...
      { text: `${data.first_pokemon_name}의 선공!`, type: "normal", ts: ts++ },
    ]
